...
function getInfo(itemStr,zip){
  ...
  var info={responseInfo:responseInfo,outcomeInfo:outcomeInfo,
...
module.exports=function package2html(zipfile) {
  ...
  var stylecontent = itemInfo.styleContent.replace(/[^}]*?{/g, x => `.${identifier} ${x}`)
  var html=`
    <html>
      <head>
        <meta charset="UTF-8" />
      ...
      <body>
        ${itemInfo.html}
        <script src="scripts/qtidisplayJs.js"></script>
        <p>pick itemInfo.json file 
          <input type="file" accept="text/json" onchange="if (this.files.length>0) handleFileSelect(this.files[0])" />
          <button onclick="getScore()">Get Score</button>
        </p>
      </body>
    </html>
    `
   fs.writeFileSync(zipfile.substr(0, zipfile.lastIndexOf(".")) + ".html", html, "utf-8")
}
